            ...
            {pagination.allPage
                ? Array.from(
                    ...
                    return i === Number(page) ?
                        <button aria-label={`${i}`} title={`${i}`} type="button" key={i} disabled={Number(page) === i} className={`w-9 h-9 md:w-12 md:h-12 flex items-center justify-center rounded-full shadow-md ${i === Number(page) ? "!bg-slate-400 dark:!bg-zinc-800/80" : "!bg-[#77adff] dark:!bg-slate-700 dark:shadow-full-dark dark:hover:shadow-none"}`}>
                            <span className="text-gray-50 pt-1">
                                {i}
                            </span>
                        </button>
            ...
            {pagination.allPage - Number(page) > 2 && (
                <>
                    ...
                    <Link aria-label={`${pagination.allPage}`} href={{ query: newQuery + `&page=${pagination.allPage}` }}>
                        <button title={`${pagination.allPage}`} type="button" className={"w-9 h-9 md:w-12 md:h-12 flex items-center justify-center rounded-full shadow-md !bg-[#77adff] dark:!bg-slate-700 dark:shadow-full-dark dark:hover:shadow-none"}>
                            <span className="text-gray-50 pt-1">
                                {pagination.allPage}
                            </span>
                        </button>
